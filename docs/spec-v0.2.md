# Open Support Agent Spec (OSAS) v0.2 — Draft

[中文版](spec-v0.2.zh-CN.md)

**Status:** Draft
**Spec version:** `0.2` (`SPEC_VERSION = "0.2"`)
**License:** Apache-2.0

This document is the normative text of OSAS v0.2, an open interoperability specification
...
- All persisted objects have `id: string`, `specVersion: "0.2"` (schema `const`), and
...
Agents interact with backends exclusively through 20 MCP tools. Each tool's input schema
...
- `specVersion: "0.2"` is a schema-level constant on every persisted object.
...
integrity. These additions are additive and optional for pre-0.1.1
implementations; introduced on the 0.1.x line (`specVersion` `"0.1"`), they
carry `specVersion` `"0.2"` on the current 0.2.x line.
...
and LLM provider/cost control. All additions are backward-compatible
extensions (introduced on the 0.1.x line with `specVersion` `"0.1"`).
...
(Zendesk ticketing, read-only Shopify commerce). All additions are
backward-compatible extensions (introduced on the 0.1.x line with
`specVersion` `"0.1"`).
...
## 15. v0.2 Phase 4/5 — minimal after-sales domain model (backward compatible)

Phase 4/5 adds four ecommerce after-sales objects, one new ActionType
(`exchange_request`), four capabilities, and four MCP tools. All additions are
backward-compatible extensions on `specVersion` `"0.2"`. Nothing in this
section executes anything: after-sales objects are records and proposals, and
`exchange_request` is fulfilled by humans only.

### 15.1 ShipmentIncident (`schemas/profiles/ecommerce/shipment-incident.json`)

A logistics exception detected on a Shipment.
...
| `specVersion` | const | yes | `"0.2"` |
| `tenantId` | Id | yes | |
| `orderId` | Id | yes | Order the incident belongs to |
| `shipmentId` | Id | yes | Shipment the incident was detected on |
| `incidentType` | enum | yes | `delayed`, `lost`, `delivered_not_received`, `damaged_in_transit` |
| `carrier` | string | no | |
| `status` | enum | yes | `open`, `investigating`, `resolved`, `closed` |
| `expectedAt` | DateTime | no | Expected delivery time relevant to the incident |
...
the model: it reflects execution outcomes and never triggers them.
...
| `tenantId` | Id | yes | |
| `orderId` | Id | yes | |
| `proposalId` | Id | no | Originating ActionProposal, when known |
| `provider` | string | no | Payment provider |
| `externalTransactionId` | string | no | Provider-side transaction id |
| `status` | enum | yes | `requested`, `processing`, `succeeded`, `failed`, `reversed` |
| `amount` | Money | yes | Integer minor units, never a float |
| `requestedAt` | DateTime | yes | |
| `completedAt` | DateTime | no | |
| `failureCode` | string | no | Provider failure code when `failed` |
| `evidenceIds` | Id[] | no | |
| `createdAt` / `updatedAt` | DateTime | yes | |

### 15.3 ItemClaim (`schemas/profiles/ecommerce/item-claim.json`)

A per-line after-sales claim. Recorded with status `submitted` and reviewed by
humans; the model never approves, rejects, or resolves a claim.
...
| `tenantId` | Id | yes | |
| `orderId` | Id | yes | |
| `lineId` | Id | yes | Order line the claim refers to |
| `claimType` | enum | yes | `damaged`, `wrong_item`, `missing_item`, `defective` |
| `quantity` | integer | yes | `>= 1` |
| `evidenceIds` | Id[] | no | |
| `status` | enum | yes | `submitted`, `under_review`, `approved`, `rejected`, `resolved` |
| `createdAt` / `updatedAt` | DateTime | yes | |

### 15.4 ExchangeRequest (`schemas/profiles/ecommerce/exchange-request.json`)

A request to exchange one order line for a replacement SKU.
...
| `tenantId` | Id | yes | |
| `orderId` | Id | yes | |
| `originalLineId` | Id | yes | Order line being exchanged |
| `replacementSku` | string | yes | |
| `replacementVariant` | string | no | |
| `inventoryStatus` | enum | no | `unknown`, `in_stock`, `out_of_stock`, `backordered` |
| `priceDelta` | Money-like | no | Replacement minus original price; **may be negative**; integer minor units, never a float |
| `returnRequired` | boolean | yes | Whether the original item must be returned |
| `status` | enum | yes | `proposed`, `pending_approval`, `approved`, `rejected`, `fulfilled`, `cancelled` |
| `evidenceIds` | Id[] | no | |
| `createdAt` / `updatedAt` | DateTime | yes | |

### 15.5 The `exchange_request` ActionType

`exchange_request` (profile `ecommerce`) is added to the ActionType registry
with these non-negotiable rules:

- **Default decision `require_approval`.** The reference demo policy carries a
  `{ actionType: "exchange_request", decision: "require_approval" }` rule. With
  no matching rule the engine fails closed (`NO_RULE` → `block`).
- **Never model-executed.** `exchange_request` is in
  `NEVER_AUTO_EXECUTE_ACTION_TYPES` (`@osas/core`). `evaluateProposal` caps its
  decision at `require_approval` (reason `NEVER_AUTO_EXECUTE`) even if a tenant
  rule says `auto_execute`, and `executeProposal` throws
  `NonExecutableActionError` (`ACTION_NOT_EXECUTABLE`) for it regardless of
  approval status — no adapter call, no state transition. Fulfillment is always
  performed by a human outside the model's execution path.
- **Evidence required / human handoff.** As a financial-class action,
  `exchange_request` requires at least one evidence reference to the original
  order: missing evidence is `block` (`INSUFFICIENT_EVIDENCE`). When inventory
  status, price delta, or original-order evidence cannot be established, the
  implementation MUST surface `capability_missing` / `require_approval` /
  a human handoff — never a fabricated success.

### 15.6 New capabilities and tools

Four capabilities join the registry (`ecommerce` profile):

| Capability | Tool | permissionRequired | adapterMethod |
|---|---|---|---|
...
`osas_ecom_create_item_claim_request` records an `ItemClaim` (`submitted`) for
human review — idempotent, never resolves anything.
`osas_ecom_create_exchange_request` is a proposal shortcut: it drafts an
`exchange_request` ActionProposal (`proposed`, `request-approval`) and never
executes. The three new adapter methods are OPTIONAL on `SupportAdapter`:
adapters that do not implement them MUST NOT declare the corresponding
capability, and calls fail with `CAPABILITY_UNSUPPORTED` rather than a fake
result. The 1:1 mapping between `TOOL_DEFINITIONS`, adapter methods, and
`schemas/tools/*.json` is enforced by the compat suite (now 20 tools, 20
spec-defined capabilities).
