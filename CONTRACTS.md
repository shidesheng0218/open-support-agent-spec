...
Machine-readable authority: `conformance/fixtures/demo-tenant.json` (timestamps encoded as relative `now±N<unit>` tokens — see `conformance/fixtures/README.md`). It is generated from `createDemoFixtures()` (`pnpm --filter @osas/mock-backend fixtures:emit`) and guarded against drift by a test in `@osas/mock-backend`; the prose below is a summary, the JSON is the authority.
